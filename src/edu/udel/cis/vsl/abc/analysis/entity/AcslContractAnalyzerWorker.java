...
public class AcslContractAnalyzerWorker {
	...
	private boolean isLvalueOfMemorySet(ExpressionNode expr) {
		if (!expressionAnalyzer.isLvalue(expr)) {
			...
			return kind == ExpressionKind.OBJECT_OR_REGION_OF || kind == ExpressionKind.NOTHING;
		}
		return true;
	}
	...
	private void processContractNode(ContractNode contractClause) throws SyntaxException {
		...
		case ASSIGNS_READS: {
			...
			for (int i = 0; i < numExpressions; i++) {
				...
				// memory type",
				// expression);
				expression.addConversion(this.conversionFactory.memoryConversion(expression.getConvertedType()));
			}
			break;
		}
		case DEPENDS: {
			...
			if (definedBehaviors.containsKey(name))
				throw this.error(
						"re-definition of behavior named as " + name + ": the previous definition was at "
								+ definedBehaviors.get(name).getBehavior().getSource().getSummary(false),
						contractClause);
			else
