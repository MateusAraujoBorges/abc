...
public class ExpressionAnalyzer {
	...
	private void processMPIEqualsNode(MPIContractExpressionNode node)
		...
		if (region0.getType().kind() != TypeKind.MEMORY)
			errorArg = region0;
		else if (region1.getType().kind() != TypeKind.MEMORY)
			errorArg = region1;
		else {
			node.setInitialType(boolType);
			return;
		...
		throw error(
				"\\mpi_equals takes two \\mpi_regions expressions as comparison operands",
				errorArg);
	}
