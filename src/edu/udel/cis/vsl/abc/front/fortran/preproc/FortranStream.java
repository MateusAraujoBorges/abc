...
public class FortranStream extends ANTLRFileStream {
	 ...
	 * characters, and in fixed form, extra characters in columns 1-6, including
	 * TAB. It also moves comments and preprocesser commands if they are in the
	 * middle of continuation lines.
	 * 
	 * Note that Holleriths in edit descriptors must be recognized, otherwise
	 * what looks like a comment will be processed incorrectly, consider the the
	 * very evil
	 * 
	...
	public FortranStream(String filename, String path, int sourceForm)
			throws IOException {
		super(path);
		this.filename = filename;
		this.filepath = path;
	...
	/**
	 * Convert characters to upper case. This is only for look ahead used in
	 * building tokens, in particular key words, the actually character buffer
	 * is unchanged so id tokens have original case.
	 */
	public int LA(int i) {
	...
	private void convertFreeFormInputBuffer() {
		...
		for (int i = 0; i < super.n; i++) {
			...
			if (col == 1) {
				...
				while ((ii = consumeFreeFormCommentLine(i, data,
						comments)) != i) {
				...
				if (continuation) {
					...
					// process a string if it exists
						...
						while (data[ii] == '&') {
							...
							col += ii - i;
							i = ii;
							ii = completeContinuedString(quoteChar, i, data,
									count, newData);
						}
						count += ii - i;
						col += ii - i;
			...
			// process all columns > 1
				...
				// process a string if it exists but retain trailing quote char
					...
					while (data[ii] == '&') {
						...
						col += ii - i;
						i = ii;
						ii = completeContinuedString(quoteChar, i, data, count,
								newData);
					}
					count += ii - i;
					col += ii - i;
				...
				// Holleriths are matched after strings so Hollerith matching
				// doesn't have
				// to worry about string, i.e, the string, "4HThis is a string".
				else if ((ii = consumeHollerith(i, data, count,
						newData)) != i) {
	...
	private void convertFixedFormInputBuffer() {
		...
		for (int i = 0; i < super.n; i++) {
			...
			if (col == 1) {
				...
				while ((ii = consumeFixedFormCommentLine(i, data,
						comments)) != i) {
					count = consumeCommentLines(count, newData, comments);
					line += 1;
					i = ii;
				...
				// Continuation checked at '\n' so no need to here, just pass
				// the character.
				// If first line is a continuation it is an error so won't need
				// to be
				// caught here. TODO - what about included files with
				// continuation, legal?

				// but I think 0 in column 6 to start is legal (gfortran and
				// ifort disagree)
				if (data[i] == '0')
					data[i] = ' ';
			}
				...
				// Holleriths are matched after strings so Hollerith matching
				// doesn't have
				// to worry about strings, i.e, the string, "4HThis is a
				// string".
				else if ((ii = consumeHollerith(i, data, count,
						newData)) != i) {
					// System.out.println("Found Hollerith character");
	...
	private int consumeFreeFormCommentLine(int i, char buf[],
	...
	/**
	 * If a comment, beginning with '!', copy the comment to comments buffer
	 * excluding the terminating '\n' character. Because the next line could be
	 * a fixed form continuation, we can't immediately consume the comment as
	 * all comments must come after all continued lines (comments can be
	 * interspersed between continued lines).
	 * 
	 * Unused for now. In future could be used to shorten code in made section
	 * when processing comments.
	 */
	@SuppressWarnings("unused")
	private int consumeFixedFormComments(int i, char buf[],
			StringBuffer comments) {
		if (i < super.n && buf[i] == '!') {
			// found comment character, copy characters up to '\n'
			do {
	 ...
	 *
	 * Check for comment characters, 'C', '*', and '!' at the start of a line. A
	 * blank line is also a comment line.
	 */
	private boolean matchFixedFormCommentLine(int i, char buf[]) {
		...
		if (matchFreeFormCommentLine(i, buf))
			return true;
	...
	/**
	 * Check for comment characters, 'C', '*', and '!' at start of a line. A
	 * blank line is also a comment line. If a comment line is found, copy the
	 * line comment to the comments buffer (without trailing '\n', and return
	 * the position of the character after the '\n' character.
	 */
	private int consumeFixedFormCommentLine(int i, char buf[],
			StringBuffer comments) {
		if (i >= super.n)
			return i;
		...
		// check for a normal comment line
		if (buf[i] == '*')
			ii = processLineForCommentChar('*', i, buf, comments);
		else if (buf[i] == 'C')
			ii = processLineForCommentChar('C', i, buf, comments);
		else if (buf[i] == 'c')
			ii = processLineForCommentChar('c', i, buf, comments);
	 ...
	 */
	private int processLineForCommentChar(char c, int i, char buf[],
			StringBuffer comments) {
		if (i >= super.n)
			return i;
	...
	private int consumePreprocessLine(int i, char buf[],
			StringBuffer comments) {
	...
	/**
	 * If the current character is '&', skip the '&' character and copy all
	 * remaining characters to the comments buffer, including '\n', to retain
	 * possible comments following the continuation character.
	 */
	private int consumeFreeFormContinuationAtEnd(int i, char buf[],
			StringBuffer comments) {
		if (i >= super.n || buf[i] != '&')
			return i;
	...
	/**
	 * Check to see if there is a continuation character as '&' the first
	 * non-blank character in a line. If there is, return the position after the
	 * '&' character.
	 */
	private int skipFreeFormContinuationAtBegin(int i, char buf[]) {
		int ii = i;

		while (ii < super.n && buf[ii] == ' ' && buf[ii] != '&')
			ii += 1;
		if (buf[ii] == '&')
			i = ii + 1;
	...
	/**
	 * Called when at a '\n' character. Look ahead for continuation character at
	 * column 6. There could be comment or preprocess lines in between so have
	 * to skip over comment lines and if they exist.
	 *
	 * The convention for a TAB character in columns 1..5 followed by a digit
	 * 1..9 is a continuation line. If TAB + '0' the '0' is ignored in the input
	 * stream. This follows DEC convention (I believe) but is non standard
	 * Fortran.
	 *
	...
	private boolean matchFixedFormContinuation(int i, char buf[]) {
		...
		//
		i += 1;
		while (matchPreprocessLine(i, buf)
				|| matchFixedFormCommentLine(i, buf)) {
			i = findCharacter('\n', i, buf);
			if (buf[i] != '\n')
				return false;
			i += 1;
		...
		// search for TAB in columns 1..5, otherwise continued position will be
		// ii
		ii = i;
		for (int j = 0; j < 5; j++) {
			if (buf[ii] == '\n')
	 ...
	 * Called when at a '\n' character. Look ahead for continuation character at
	 * column 6. There could be comment or preprocess lines in between so have
	 * to search for comment lines and remove them if they exist.
	 *
	 * The convention for a TAB character in columns 1..5 followed by a digit
	 * 1..9 is a continuation line. If TAB + '0' the '0' is ignored in the input
	 * stream. This follows DEC convention (I believe) but is non standard
	 ...
	 * WARNING, don't go beyond length of stream, super.n
	 */
	private int consumeFixedFormContinuation(int i, char buf[],
			StringBuffer comments) {
		...
		if (matchFixedFormCommentLine(i, buf)) {
			return (consumeFixedFormCommentLine(i, buf, comments) - 1); // retain
																		// the
																		// '\n'
		}
	...
	/**
	 * Check for a Hollerith following the current character position. First
	 * must ensure that it's not an identifier, "var_2Hxx", so look for
	 * preceding character, ' ', '(', ',' (and perhaps more). Then look for
	 * digit string, n, followed by 'H'|'h' and then n characters (none of which
	 * is \'n'). Perhaps we want to change Hollerith to a quoted string. In any
	 * case, copy string representation to newBuf.
	 *
	 * We would like to be conservative while matching Hollerith's. Examples
	 * showing characters that can precede a Hollerith constant: " 1Hx", "=1Hx",
	 * ".eq.1Hx", "(1Hx", "-1Hx", ",1Hx". Note Hollerith constants have no data
	 * type; they assume a numeric type based on the way they are used. They
	 * cannot assume a character data type and cannot be used where a character
	 * value is expected (from a DEC manual for F77, I believe). Not sure this
	 * applies to Hollerith edit descriptors.
	 *
	 * Assume that strings have been matched so a Hollerith-like constant within
	 * a string doesn't have to been considered.
	 *
	...
	/**
	 * Complete the processing of a string that is continued across multiple
	 * lines.
	 */
	private int completeContinuedString(char quoteChar, int i, char buf[],
			int count, char newBuf[]) {
		int i0 = i;
		...
		//
		// if (i >= super.n || buf[i] != '&') {
		// // first non-blank character is part of the continued string so back
		// up to get it
		// i -= 1;
		//
		// //CER System.err.println("Terminating '&' not found in continued
		// string at character position " + i);
		// //CER return i0;
		// }
		...
		// skip trailing '&'
		//
		// NOTE: gfortran doesn't require the terminating character (warns with
		// -Wall)
		...
		do {
			...
			if (i < super.n - 1 && buf[i] == quoteChar
					&& buf[i + 1] == quoteChar) {
				newBuf[count++] = buf[i++];
				newBuf[count++] = buf[i++];
			}
		} while (i < super.n && buf[i] != quoteChar && buf[i] != '&'
				&& buf[i] != '\n');
	...
	/**
	 * Check for the beginning of a string at this character position. If found
	 * copy the characters of the string into newBuf, except for the terminating
	 * quote character. A string may be continued, if so it continues after the
	 * '&' character on the next line; return the position of the trailing '&'.
	 * If a string doesn't terminate it's an error, return '\n' position.
	 */
	private int consumeFreeFormString(int i, char buf[], int count,
			char newBuf[]) {
		if (i >= super.n)
			return i;
		...
		char quote_char = buf[i];
		if (quote_char != '"' && quote_char != '\'')
			return i; // not the start of a string

		do {
			newBuf[count++] = buf[i++];
			// look for two quote chars in a row, if found copy both
			if (i < super.n - 1 && buf[i] == quote_char
					&& buf[i + 1] == quote_char) {
				newBuf[count++] = buf[i++];
				newBuf[count++] = buf[i++];
			}
			// look for continuation character as last non-blank character and
			// if found, return the '&' position so caller can process
			// continuation
			if (buf[i] == '&') {
				int ii = i;
				while (buf[++ii] == ' ')
					;
				if (buf[ii] != '\n') {
					// '&' not a continuation, just part of the string, so just
					// copy it
					newBuf[count++] = buf[i++];
				}
			}
	 ...
	 * Check for the beginning of a string at this character position. If found
	 * copy the characters of the string into newBuf, except for the terminating
	 * quote character. If a string doesn't terminate it's an error, return '\n'
	 * position.
	 */
	private int consumeFixedFormString(int i, char buf[], int count,
			char newBuf[]) {
		if (i >= super.n)
			return i;
