...
//      } while(nextChar != '\'' && nextChar != '"');
      ...
      while (lineIndex < (lineLength-1) && foundClosingParen == false) {
         ...
         if (descIndex != -1 || 
            ...
            if (line.charAt(descIndex) == ',') {
               ...
               tokens.addToken(
                  tokens.createToken(FortranLexer.T_COMMA, ",", lineNum, charPos, sysFormation));
            } else if (line.charAt(descIndex) == ')') {
               tokens.addToken(
                  tokens.createToken(FortranLexer.T_RPAREN, ")", lineNum, charPos, sysFormation));
            } else {
               if (line.charAt(descIndex) == ':') {
                  termString = new String(":");
               } else if (line.charAt(descIndex) == '/') {
               ...
               if (termString != null) {
                  ...
                  tokens.addToken(
                     tokens.createToken(FortranLexer.T_CONTROL_EDIT_DESC, 
                                        termString, lineNum, charPos, sysFormation));
               }
            }
            ...
            if (startIndex != lineIndex) {
               ...
               tokens.addToken(
                  tokens.createToken(FortranLexer.T_DIGIT_STRING, 
                                     ...
                                     lineNum, charPos-(lineIndex-startIndex), sysFormation));
            }

            // make sure we're on a left paren
            if (line.charAt(lineIndex) == '(') {
               tokens.addToken(
                  tokens.createToken(FortranLexer.T_LPAREN, "(", lineNum, charPos, sysFormation));
               charPos++;
   ...
   private int fixupFormatStmt(int start, int lineEnd) {
      ...
      tokens.addToken(
         tokens.createToken(FortranLexer.T_EOS, "\n", lineNum, charPos+lineIndex, sysFormation)
      );
   ...
   private void fixupLabeledEndDo(int lineStart, int lineEnd) {
      ...
      if (doLabels.empty() == false) {
         ...
         if (labelsMatch(doLabelString, firstToken.getText()) == true) {
            ...
            tokens.addToken(1, FortranLexer.T_LABEL_DO_TERMINAL, labeledDoText, sysFormation);
            ...
            while (doLabels.empty() == false &&
               ...
               if (tokens.appendToken(FortranLexer.T_DIGIT_STRING, 
                                      new String(firstToken.getText()), sysFormation) == false ||
                   tokens.appendToken(FortranLexer.T_LABEL_DO_TERMINAL, labeledDoText, sysFormation) == false ||
                   tokens.appendToken(FortranLexer.T_CONTINUE, new String("CONTINUE"), sysFormation) == false ||
                   tokens.appendToken(FortranLexer.T_EOS, null, sysFormation) == false) {
                  // should we exit here??
   ...
   private boolean matchOneLineStmt(int lineStart, int lineEnd) {
      ...
      if (tokenType == FortranLexer.T_WHERE ||
         ...
         if (tokens.currLineLA(lineStart+2) == FortranLexer.T_LPAREN) {
            ...
            if (tokenType == FortranLexer.T_WHERE) {
               tokens.addToken(lineStart, FortranLexer.T_WHERE_STMT, "__T_WHERE_STMT__", sysFormation);
            } else if (tokenType == FortranLexer.T_IF) {
               tokens.addToken(lineStart, FortranLexer.T_IF_STMT, "__T_IF_STMT__", sysFormation);
            } else {
               tokens.addToken(lineStart, FortranLexer.T_FORALL_STMT, "__T_FORALL_STMT__", sysFormation);
            }
   ...
   private boolean matchAssignStmt(int lineStart, int lineEnd)
   {
      ...
      if ( identOffset != -1 ) {
         ...
         if ( assignType == FortranLexer.T_EQUALS ) {
            tokens.addToken(lineStart, FortranLexer.T_ASSIGNMENT_STMT, "__T_ASSIGNMENT_STMT__", sysFormation);
         }
         else if ( assignType == FortranLexer.T_EQ_GT ) {
            tokens.addToken(lineStart, FortranLexer.T_PTR_ASSIGNMENT_STMT, "__T_PTR_ASSIGNMENT_STMT__", sysFormation);
         }
      ...
      for (int i = lineStart; i < lineEnd; i++) {
         ...
         if (tokens.currLineLA(i+1) == FortranLexer.T_PERIOD) {
            ...
            if (tokens.currLineLA(i+2) == FortranLexer.T_IDENT &&
               ...
               // reflects new token
               tokens.add(i, tokens.createToken(type, "." + text + ".", line, col, sysFormation));
