...
public class FortranLexicalPrepass {
	...
	private Formation sysFormation;
	...
	public FortranLexicalPrepass(FortranLexer lexer,
		...
		this.sysFormation = lexer.getFormation();
	}
	 ...
	 * (tokens.getToken(start).getType() == FortranLexer.T_ASSIGNMENT &&
	 * (start+3 < end) && tokens.getToken(start+1).getType() ==
	 * FortranLexer.T_LPAREN && tokens.getToken(start+2).getType() ==
	 * FortranLexer.T_EQUALS) { return true; } else { return false; } } // end
	 * isAssignment() END OBSOLETE
	 ********/

	/*******
	 * OBSOLETE private boolean isOperator(int start, int end) { if
	 * (tokens.getToken(start).getType() == FortranLexer.T_OPERATOR && (start+3
	 * < end) && tokens.getToken(start+1).getType() == FortranLexer.T_LPAREN &&
	 * tokens.getToken(start+2).getType() == FortranLexer.T_DEFINED_OP &&
	 * tokens.getToken(start+3).getType() == FortranLexer.T_RPAREN) { return
	...
	private int convertToIdents(int start, int end) {
		...
		for (i = start; i < end; i++) {
			...
			if (lexer.isKeyword(tk) == true) {
				...
				// i.e.,
				// ASSIGNMENT(=) OPERATOR(T_DEFINED_OP), READ(UN/FORMATTED),
				// WRITE()
	 ...
	 * constants. Actually, the lexer already sucks up quotes (single and
	 * double) into the T_CHAR_CONSTANT tokens that it creates, so no need to
	 * consider here.
	 */
		...
		if ((tkType == FortranLexer.T_IF || tkType == FortranLexer.T_ELSEIF)
			...
			if (tokens.currLineLA(rparenOffset + 1) == FortranLexer.T_THEN) {
				...
				return true;
					...
					&& tokens.currLineLA(
							rparenOffset + 1) == FortranLexer.T_DIGIT_STRING) {
				...
				tokens.addToken(lineStart, FortranLexer.T_ARITHMETIC_IF_STMT,
						"__T_ARITHMETIC_IF_STMT__", sysFormation);
	...
	private boolean matchDataDecl(int lineStart, int lineEnd) {
		...
		if (isIntrinsicType(tokenType) == true || isPrefixToken(tokenType)
				|| ((tokenType == FortranLexer.T_TYPE
						...
						&& tokens.currLineLA(
								lineStart + 2) == FortranLexer.T_LPAREN)) {
	...
	private boolean matchSub(int lineStart, int lineEnd) {
		...
		if (bindOffset != -1) {
			// use the T_BIND token as a marker for the end of the subroutine
			// name and any args.
	...
	private boolean matchEnd(int lineStart, int lineEnd) {
		...
		if (tokenType == FortranLexer.T_END) {
			if (lineEnd > 2) {
				...
				// "end with team"
				int nextToken = tokens.currLineLA(lineStart + 2); // the next
																	// token
				int nextNextToken = tokens.currLineLA(lineStart + 3); // the
																		// next
																		// of
																		// the
																		// next
				...
				if (nextToken == FortranLexer.T_BLOCK && tokens
						.currLineLA(lineStart + 3) == FortranLexer.T_DATA) {
					// end-block-data-stmt
					...
					// Laksono 2009.03.03:
					// We have the perfect match of "end with team", so push the
					// token 4 steps
		...
		if (tokens.currLineLA(lineStart + 1) == FortranLexer.T_BLOCK) {
			if (tokens.currLineLA(lineStart + 2) == FortranLexer.T_EOS) {
				...
				return true;
					...
					.currLineLA(lineStart + 2) == FortranLexer.T_DATA) {
				// T_BLOCK T_DATA (T_IDENT)? T_EOS
				if ((lineEnd >= (lineStart + 3)) && lexer
						.isKeyword(tokens.currLineLA(lineStart + 3)) == true) {
					// lookAhead 3 is index 2
					tokens.getToken(lineStart + 2)
							.setType(FortranLexer.T_IDENT);
				}
			...
			return false;
				...
				.currLineLA(lineStart + 1) == FortranLexer.T_BLOCKDATA) {
	 ...
	 * assumes that the T_EOS tokens in a multi-line statement are removed for
	 * all lines except the last. This allows this function to simply test if
	 * the first token on the line is a digit string.
	 */
	 ...
	 * first line of the included file. So search for T_INCLUDE T_INCLUDE_NAME
	 * and return true if they are found. The T_CHAR_CONSTANT token following
	 * T_INCLUDE is assigned to channel 99 and is not seen here.
	 */
	private boolean matchInclude(int lineStart, int lineEnd) {
		if (tokens.currLineLA(lineStart + 1) == FortranLexer.T_INCLUDE && tokens
				.currLineLA(lineStart + 2) == FortranLexer.T_INCLUDE_NAME) {
			return true;
		} else {
			return false;
		}
	...
	private boolean matchProcStmt(int lineStart, int lineEnd) {
		...
		if (tokens.currLineLA(lineStart + 1) == FortranLexer.T_PROCEDURE
			...
			if (colonOffset != -1) {
				...
				&& tokens.currLineLA(
						lineStart + 2) == FortranLexer.T_PROCEDURE) {
	 ...
	 * access-id-list. An access-id can be T_IDENT or KEYWORD ( stuff ) where
	...
	private boolean matchAttrStmt(int lineStart, int lineEnd) {
		...
		case FortranLexer.T_BIND:
			// find the closing paren, starting at first location after the left
			// paren.
			// What follows it is optional :: and the ident(s). The T_BIND and
			// T_LPAREN
		...
		case FortranLexer.T_PUBLIC:
			...
			// access-id-list.
			// An access-id can be T_IDENT or KEYWORD ( stuff ) where KEYWORD is
			// one of
			...
			if (tokens.currLineLA(lineStart + 2) != FortranLexer.T_NONE) {
				do {
					...
					if (lParenOffset != -1) {
						...
						// either way, we can
						// convert anything that's not T_KIND or T_LEN to an
						// ident because T_KIND
						...
						for (int i = lParenOffset; i < rParenOffset; i++) {
							if (lexer.isKeyword(tokens.currLineLA(i + 1))
									&& tokens.currLineLA(
											i + 1) != FortranLexer.T_KIND
									&& tokens.currLineLA(
											i + 1) != FortranLexer.T_LEN) {
								tokens.getToken(i)
										.setType(FortranLexer.T_IDENT);
							}
						}

						// There could be another set of parens, if the first
						// set (above) was for
						// the kind selector, then the second set would be for
						// the letter
						// designator(s) (required).
						if (tokens.currLineLA(
								rParenOffset + 1) == FortranLexer.T_LPAREN) {
						...
						.currLineLA(lineStart + 1) != FortranLexer.T_EOS);
			}
			break;
	...
	private boolean isOpenParen(int token) {
		return token == FortranLexer.T_LPAREN
				|| token == FortranLexer.T_LBRACKET;
	} // end isOpenParen()
	...
	private boolean isCloseParen(int token) {
		return token == FortranLexer.T_RPAREN
				|| token == FortranLexer.T_RBRACKET;
	} // end isCloseParen()
	...
	private int arrayConstructorIndices(int[] indices) {
		...
		// look for '(' '/'
		else if (tk.getType() == FortranLexer.T_LPAREN && tokens
				.getToken(start + 1).getType() == FortranLexer.T_SLASH) {
			indices[0] = start + 1;
			indices[1] = tokens.findToken(start + 2, FortranLexer.T_SLASH);
			if (indices[1] != -1) {
				indices[1] += 1;
				if (tokens.getToken(indices[1])
						.getType() != FortranLexer.T_RPAREN) {
					indices[0] = -1;
					indices[1] = -1;
				}
			}
	...
	private int fixupArrayConstructor(int[] indices) {
		...
		if (matchTypeSpec(indices) != -1) {
			if (tokens.getToken(indices[1] + 1)
					.getType() == FortranLexer.T_COLON_COLON) {
	...
	private int matchClosingParen(int offset) {
		...
		do {
			...
			if ((tmpTokenType == FortranLexer.T_EOS
					|| tmpTokenType == FortranLexer.EOF) && nestingLevel != 0) {
	...
	private int fixupDeclTypeSpec(int lineStart, int lineEnd) {
		...
		if (tokens.getToken(lineStart).getType() == FortranLexer.T_TYPE
				|| tokens.getToken(lineStart)
						.getType() == FortranLexer.T_CLASS) {
			int rparenOffset = -1;
			// left-paren is next token (or we're in trouble)
			if (tokens.getToken(lineStart + 1)
					.getType() != FortranLexer.T_LPAREN) {
		...
		if (tokens.getToken(lineStart + 1).getType() == FortranLexer.T_LPAREN) {
			...
			kindTokenOffset = tokens.findToken(lineStart + 1,
					FortranLexer.T_KIND);
			lenTokenOffset = tokens.findToken(lineStart + 1,
					FortranLexer.T_LEN);
	...
	private boolean isIntrinsicType(int type) {
		if (type == FortranLexer.T_INTEGER || type == FortranLexer.T_REAL
	...
	private int skipTypeSpec(int lineStart) {
		...
		if (isIntrinsicType(firstToken) == true
			...
			if (firstToken == FortranLexer.T_CHARACTER && tokens
					.currLineLA(lineStart + 2) == FortranLexer.T_ASTERISK) {
				lineStart += 2;
			}
	...
	private boolean isPrefixToken(int token) {
		if (token == FortranLexer.T_PURE || token == FortranLexer.T_ELEMENTAL
	...
	private boolean isValidControlEditDesc(String line, int lineIndex) {
		...
		if (firstChar == ':' || firstChar == '/' || firstChar == 'p'
			...
			if (firstChar == 's')
				...
				return false;
				else if (firstChar == 't' && (isDigit(secondChar) != true && secondChar != 'l' && secondChar != 'r'))
				return false;
				else if (firstChar == 'b' && (secondChar != 'n' && secondChar != 'z'))
				return false;
				else if (firstChar == 'r' && (secondChar != 'u' && secondChar != 'd' && secondChar != 'z' && secondChar != 'n' && secondChar != 'c' && secondChar != 'p'))
				return false;
	...
	// Return the index for the end of the char string. The lexer already
	// verified that
	...
	*/
	private int parseFormatString(String line, int lineIndex, int lineNum,
		...
		while (lineIndex < (lineLength - 1) && foundClosingParen == false) {
					...
					if (descIndex != -1) {
						...
						if ((descIndex - lineIndex) > 0
							...
							tokens.addToken(tokens.createToken(
									FortranLexer.T_CONTROL_EDIT_DESC,
									line.substring(lineIndex, descIndex),
									lineNum, charPos, sysFormation));
			...
			if (descIndex != -1 || (descIndex == -1
				...
				if (line.charAt(descIndex) == ',') {
					...
					tokens.addToken(tokens.createToken(FortranLexer.T_COMMA,
							",", lineNum, charPos, sysFormation));
				} else if (line.charAt(descIndex) == ')') {
					tokens.addToken(tokens.createToken(FortranLexer.T_RPAREN,
							")", lineNum, charPos, sysFormation));
				} else {
					if (line.charAt(descIndex) == ':') {
						termString = new String(":");
					...
					if (termString != null) {
						...
						tokens.addToken(tokens.createToken(
								FortranLexer.T_CONTROL_EDIT_DESC, termString,
								lineNum, charPos, sysFormation));
					}
				}
		 ...
		 * example: 004 format(//) would create a T_CONTROL_EDIT_DESCRIPTOR for
		 * the last /, and then advance the index to the ')'. however, the
		 * rparen is not a format item, and so is not considered in the above
		 * while loop. that is why a T_RPAREN is added here if necessary.
		 */
	 ...
	 * format-specification are just converted by the lexer to identifiers so
	...
	private int fixupFormat(int start, int lineEnd) {
		...
		if (tk == FortranLexer.T_ASTERISK
				|| tk == FortranLexer.T_DIGIT_STRING) {
			return start + 1;
		}
	...
	private int fixupFormatStmt(int start, int lineEnd) {
		...
		// first, copy the starting tokens to the new line (label T_FORMAT,
		// etc.)
		for (i = 0; i < start; i++) {
			// adds to the end
		...
		tokens.addToken(tokens.createToken(FortranLexer.T_EOS, "\n", lineNum,
				charPos + lineIndex, sysFormation));
	...
	/**
	 * Match one of the following tokens for an IO statement: T_CLOSE, T_OPEN,
	 * T_READ, T_FLUSH, T_REWIND, T_WRITE, T_INQUIRE, T_FORMAT, T_PRINT
	 */
	private boolean matchIOStmt(int lineStart, int lineEnd) {
			...
			if (tokens.currLineLA(lineStart + 2) == FortranLexer.T_LPAREN) {
				...
				if (tokenType == FortranLexer.T_INQUIRE) {
					...
					if (rparenOffset != -1 && (rparenOffset < (lineEnd - 1))) {
						if (tokens.currLineLA(
								rparenOffset + 1) != FortranLexer.T_EOS) {
							// add a token saying it must be alt2
							tokens.addToken(lineStart,
									FortranLexer.T_INQUIRE_STMT_2,
									"__T_INQUIRE_STMT_2__", sysFormation);
					...
					&& tokens.currLineLA(
							lineStart + 2) != FortranLexer.T_EQUALS) {
		...
		if (identOffset != -1) {
			...
			if (lineStart > 0 && tokens
					.currLineLA(lineStart) == FortranLexer.T_DIGIT_STRING) {
				fixupLabeledEndDo(lineStart, lineEnd);
			}
			...
			if (labelsMatch(doLabelString, firstToken.getText()) == true) {
				...
				while (doLabels.empty() == false
					...
					if (tokens.appendToken(FortranLexer.T_DIGIT_STRING,
							...
							sysFormation) == false
							|| tokens.appendToken(
									FortranLexer.T_LABEL_DO_TERMINAL,
									labeledDoText, sysFormation) == false
							|| tokens.appendToken(FortranLexer.T_CONTINUE,
									new String("CONTINUE"),
									sysFormation) == false
							|| tokens.appendToken(FortranLexer.T_EOS, null,
									sysFormation) == false) {
						// should we exit here??
		...
		if (tokenType == FortranLexer.T_GO) {
			...
			if (colonOffset != -1) {
				...
				tokens.addToken(lineStart, FortranLexer.T_ALLOCATE_STMT_1,
						"__T_ALLOCATE_STMT_1__", sysFormation);
				lineStart++;
	...
	private boolean matchSingleTokenStmt(int lineStart, int lineEnd) {
		...
		if (firstToken == FortranLexer.T_COMMON
				...
				|| (firstToken == FortranLexer.T_CLASS && tokens
						.currLineLA(lineStart + 2) != FortranLexer.T_DEFAULT)
			...
			if (firstToken == FortranLexer.T_SELECT
					|| (firstToken == FortranLexer.T_CASE && tokens.currLineLA(
							lineStart + 2) == FortranLexer.T_DEFAULT)) {
				convertToIdents(lineStart + 2, lineEnd);
	...
	private boolean matchDoStmt(int lineStart, int lineEnd) {
		...
		if (tokens.currLineLA(lineStart + 2) == FortranLexer.T_DIGIT_STRING) {
			// Wwh
			CommonCivlcToken tempToken = new CommonCivlcToken(
					tokens.getToken(lineStart + 1), sysFormation);
		...
		//
		if (tokens.currLineLA(keywordOffset) == FortranLexer.T_WHILE || tokens
				.currLineLA(keywordOffset) == FortranLexer.T_CONCURRENT) {
	...
	private boolean matchOneLineStmt(int lineStart, int lineEnd) {
		...
		tokenType = tokens.currLineLA(lineStart + 1);
		if (tokenType == FortranLexer.T_WHERE || tokenType == FortranLexer.T_IF
	...
	private boolean matchGenericBinding(int lineStart, int lineEnd) {
		if (tokens.currLineLA(lineStart + 1) == FortranLexer.T_GENERIC) {
			...
			// if the next token is a T_OPERATOR, T_ASSIGNMENT, T_READ, or
			// T_WRITE
	 ...
	 * Token token; int i = 0;
	 * 
	 * if(startsWithKeyword == true) { do { System.out.println(
	 * "fixed-format line must start with keyword"); tokens.printPackedList();
	 * buffer = buffer.append(tokens.getToken(lineStart+i).getText());
	 * 
	 * ANTLRStringStream charStream = new
	 * ANTLRStringStream(buffer.toString().toUpperCase()); FortranLexer myLexer
	 * = new FortranLexer(charStream);
	 * 
	 * // System.out.println("trying to match the string: " + //
	 * buffer.toString().toUpperCase() + //
	 * " as keyword for fixed-format continuation"); token =
	 * myLexer.nextToken(); // System.out.println(
	 * "lexer said next token.getText() is: " + // token.getText()); //
	 ...
	 * " on line: " + tmpToken.getLine()); tokens.set(j, tmpToken); }
	 * 
			...
			if (tokens.currLineLA(i + 1) == FortranLexer.T_PERIOD_EXPONENT) {
				...
				tokens.getToken(i).setType(FortranLexer.T_REAL_CONSTANT);
					...
					&& (tokens.currLineLA(i + 2) == FortranLexer.T_PERIOD
							|| tokens.currLineLA(
									i + 2) == FortranLexer.T_PERIOD_EXPONENT)) {
				...
				tokens.add(i, tokens.createToken(FortranLexer.T_REAL_CONSTANT,
						newTokenText.toString(), line, col, sysFormation));
	...
	public void performPrepass() {
		...
		while (tokens.LA(1) != FortranLexer.EOF) {
			...
			if (matchInclude(lineStart, lineLength) == true) {
				...
				if (tokens.getToken(lineStart).getType() == FortranLexer.T_EOF)
					lineStart += 1;
			}
