...
public class SideEffectRemover extends BaseTransformer {
	...
	/**
	 * An array lambda expression shouldn't have side-effects.
	 * 
	 * @param expression
	 * @param isVoid
	 * @return
	 */
	private ExprTriple translateArrayLambdaExpression(ArrayLambdaNode expression) {
	 ...
	 * @return
	 */
	private ExprTriple translateQuantifiedExpression(QuantifiedExpressionNode expression) {
		// should never have side-effects: check it in Analyzer
		assert (expression.isSideEffectFree(false));
		return new ExprTriple(expression);
	}
